...
from . import SetupFactory, GradManager, DistributedRunner
...
class DistributedTrainer(DistributedRunner):
    ...
    async def _run(self):
        ...
        await setup_factory.download_dataset()        
        self.train_data = self._create_dataloader(setup_factory.create_train_dataset())
        self.val_data = self._create_dataloader(setup_factory.create_val_dataset()) # TODO
        
        model = setup_factory.create_model()
        self.optimizer = setup_factory.create_optimizer(model)
        ...
        self.checkpoints_folder_path = config.train.checkpoints_folder_path
